# Dropdown — Implementation Notes (pre-implementation stub)
...
- **Category**: EASY
- **Angular Material / CDK candidate**: `MatSelect` (`select.d.ts`)
- **Notes**: Mantine's `Dropdown` = Mantine's `Select` per mantine-adapter's own `Dropdown.tsx` (naming translation). `MatSelect` is a strong match: `multiple`, `disabled`, `placeholder`, `required`, `panelClass`, `compareWith`, works inside `<mat-form-field>`.
