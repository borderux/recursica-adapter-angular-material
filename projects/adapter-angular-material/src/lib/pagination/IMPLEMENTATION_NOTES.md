# Pagination — Implementation Notes (pre-implementation stub)
...
- **Angular Material / CDK candidate**: `MatPaginator` (`paginator.d.ts`)
- **Notes**: Real mismatch, not a naming one: `MatPaginator` is purpose-built for a data-table (`length`/`pageSize`/`pageSizeOptions` + prev/next + an optional page-size `<mat-select>`), not a generic numbered-page-pill control the way Mantine's `Pagination` is. Likely needs a custom-built numbered control rather than wrapping `MatPaginator` directly.
