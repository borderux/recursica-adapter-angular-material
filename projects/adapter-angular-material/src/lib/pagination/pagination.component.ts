import { NgTemplateOutlet } from "@angular/common";
import {
  Component,
  EventEmitter,
  Input,
  OnInit,
  Output,
  ViewEncapsulation,
  signal,
} from "@angular/core";
import {
  RecursicaOverStyled,
  resolveOverStyle,
} from "../utils/recursica-over-styled";
import {
  PAGINATION_DOTS,
  RecursicaPaginationRangeItem,
  computePaginationRange,
} from "./pagination-range";

export type RecursicaPaginationIconType = "next" | "prev" | "first" | "last";

const ICON_PATHS: Record<RecursicaPaginationIconType, string> = {
  next: "M8.781 8l-3.3-3.3.943-.943L10.667 8l-4.243 4.243-.943-.943 3.3-3.3z",
  prev: "M7.219 8l3.3 3.3-.943.943L5.333 8l4.243-4.243.943.943-3.3 3.3z",
  first:
    "M6.85355 3.85355C7.04882 3.65829 7.04882 3.34171 6.85355 3.14645C6.65829 2.95118 6.34171 2.95118 6.14645 3.14645L2.14645 7.14645C1.95118 7.34171 1.95118 7.65829 2.14645 7.85355L6.14645 11.8536C6.34171 12.0488 6.65829 12.0488 6.85355 11.8536C7.04882 11.6583 7.04882 11.3417 6.85355 11.1464L3.20711 7.5L6.85355 3.85355ZM12.8536 3.85355C13.0488 3.65829 13.0488 3.34171 12.8536 3.14645C12.6583 2.95118 12.3417 2.95118 12.1464 3.14645L8.14645 7.14645C7.95118 7.34171 7.95118 7.65829 8.14645 7.85355L12.1464 11.8536C12.3417 12.0488 12.6583 12.0488 12.8536 11.8536C13.0488 11.6583 13.0488 11.3417 12.8536 11.1464L9.20711 7.5L12.8536 3.85355Z",
  last: "M2.14645 11.1464C1.95118 11.3417 1.95118 11.6583 2.14645 11.8536C2.34171 12.0488 2.65829 12.0488 2.85355 11.8536L6.85355 7.85355C7.04882 7.65829 7.04882 7.34171 6.85355 7.14645L2.85355 3.14645C2.65829 2.95118 2.34171 2.95118 2.14645 3.14645C1.95118 3.34171 1.95118 3.65829 2.14645 3.85355L5.79289 7.5L2.14645 11.1464ZM8.14645 11.1464C7.95118 11.3417 7.95118 11.6583 8.14645 11.8536C8.34171 12.0488 8.65829 12.0488 8.85355 11.8536L12.8536 7.85355C13.0488 7.65829 13.0488 7.34171 12.8536 7.14645L8.85355 3.14645C8.65829 2.95118 8.34171 2.95118 8.14645 3.14645C7.95118 3.34171 7.95118 3.65829 8.14645 3.85355L11.7929 7.5L8.14645 11.1464Z",
};

const LABELS: Record<RecursicaPaginationIconType, string> = {
  next: "Next",
  prev: "Prev",
  first: "First",
  last: "Last",
};

/**
 * Recursica `Pagination` — Angular Material adapter.
 *
 * REAL implementation (`docs/CREATING_AN_ADAPTER.md` step 10). The stub's
 * own `IMPLEMENTATION_NOTES.md` already confirmed a real mismatch, not a
 * naming one: `MatPaginator` is purpose-built for a data-table (`length`/
 * `pageSize`/`pageSizeOptions` + prev/next + an optional page-size
 * `<mat-select>`), not a generic numbered-page-pill control the way
 * Mantine's `Pagination` is. Re-confirmed at build time — hand-built
 * instead, no adoption of `MatPaginator`.
 *
 * ## Page-range/ellipsis math: a direct port of `@mantine/hooks`' `usePagination`
 *
 * `pagination-range.ts`'s `computePaginationRange` is read directly from
 * `use-pagination.mjs`'s compiled source, not reimplemented from a
 * description of "reasonable" pagination behavior — the reference's own
 * `<Pagination total={10} />` golden story is rendered against this exact
 * algorithm, so reproducing it verbatim (siblings=1, boundaries=1 defaults)
 * is required for parity, not just a nice-to-have.
 *
 * ## Only the top-level `<Pagination>` shape — confirmed, not the compound `Root`/`Items`/etc.
 *
 * Confirmed by reading `Pagination.stories.tsx` directly: all 3 golden
 * stories use only `<Pagination total withEdges withControls withLabels>`
 * — none exercise the reference's own `Pagination.Root`/`.Items`/`.Control`/
 * `.Dots`/`.Next`/`.Previous`/`.First`/`.Last` "advanced composition"
 * sub-components. Building those with no golden coverage would be
 * speculative scope, the same reasoning `Modal`'s own granular
 * sub-components were skipped for.
 */
@Component({
  selector: "rec-pagination",
  encapsulation: ViewEncapsulation.Emulated,
  styleUrl: "./pagination.component.css",
  imports: [NgTemplateOutlet],
  template: `
    <div
      class="root"
      [class]="resolvedOverStyle.class"
      [style]="resolvedOverStyle.style"
    >
      @if (withEdges) {
        <button
          type="button"
          class="control"
          data-variant="text"
          [disabled]="disabled || currentValue <= 1"
          [attr.aria-label]="'First page'"
          (click)="setPage(1)"
        >
          <ng-container
            [ngTemplateOutlet]="iconTpl"
            [ngTemplateOutletContext]="{ $implicit: 'first' }"
          />
        </button>
      }
      @if (withControls) {
        <button
          type="button"
          class="control"
          data-variant="text"
          [disabled]="disabled || currentValue <= 1"
          [attr.aria-label]="'Previous page'"
          (click)="setPage(currentValue - 1)"
        >
          <ng-container
            [ngTemplateOutlet]="iconTpl"
            [ngTemplateOutletContext]="{ $implicit: 'prev' }"
          />
        </button>
      }

      @for (item of rangeItems; track $index) {
        @if (item === dots) {
          <span class="dots" aria-hidden="true">&hellip;</span>
        } @else {
          <button
            type="button"
            class="control"
            [attr.data-active]="item === currentValue ? '' : null"
            [attr.aria-current]="item === currentValue ? 'page' : null"
            [attr.aria-label]="'Page ' + item"
            [disabled]="disabled"
            (click)="setPage(item)"
          >
            {{ item }}
          </button>
        }
      }

      @if (withControls) {
        <button
          type="button"
          class="control"
          data-variant="text"
          [disabled]="disabled || currentValue >= total"
          [attr.aria-label]="'Next page'"
          (click)="setPage(currentValue + 1)"
        >
          <ng-container
            [ngTemplateOutlet]="iconTpl"
            [ngTemplateOutletContext]="{ $implicit: 'next' }"
          />
        </button>
      }
      @if (withEdges) {
        <button
          type="button"
          class="control"
          data-variant="text"
          [disabled]="disabled || currentValue >= total"
          [attr.aria-label]="'Last page'"
          (click)="setPage(total)"
        >
          <ng-container
            [ngTemplateOutlet]="iconTpl"
            [ngTemplateOutletContext]="{ $implicit: 'last' }"
          />
        </button>
      }
    </div>

    <ng-template #iconTpl let-type>
      @if (withLabels && (type === "next" || type === "last")) {
        <span class="iconWithLabel">
          <span>{{ iconLabel(type) }}</span>
          <svg class="baseIcon" viewBox="0 0 16 16" aria-hidden="true">
            <path [attr.d]="iconPath(type)" fill="currentColor" />
          </svg>
        </span>
      } @else if (withLabels) {
        <span class="iconWithLabel">
          <svg class="baseIcon" viewBox="0 0 16 16" aria-hidden="true">
            <path [attr.d]="iconPath(type)" fill="currentColor" />
          </svg>
          <span>{{ iconLabel(type) }}</span>
        </span>
      } @else {
        <svg class="baseIcon" viewBox="0 0 16 16" aria-hidden="true">
          <path [attr.d]="iconPath(type)" fill="currentColor" />
        </svg>
      }
    </ng-template>
  `,
})
export class PaginationComponent implements RecursicaOverStyled, OnInit {
  @Input() total = 0;
  @Input() value?: number;
  @Input() defaultValue = 1;
  @Output() valueChange = new EventEmitter<number>();

  @Input() siblings = 1;
  @Input() boundaries = 1;
  @Input() withEdges = false;
  @Input() withControls = true;
  @Input() withLabels = false;
  @Input() disabled = false;

  @Input() overStyled = false;
  @Input() overClass?: string;
  @Input() overStyle?: Record<string, string>;

  readonly dots = PAGINATION_DOTS;

  private readonly _uncontrolledValue = signal(1);

  ngOnInit(): void {
    this._uncontrolledValue.set(this.defaultValue);
  }

  get currentValue(): number {
    return this.value !== undefined ? this.value : this._uncontrolledValue();
  }

  get rangeItems(): RecursicaPaginationRangeItem[] {
    return computePaginationRange(
      this.total,
      this.currentValue,
      this.siblings,
      this.boundaries,
    );
  }

  get resolvedOverStyle(): {
    class: string | null;
    style: Record<string, string> | null;
  } {
    return resolveOverStyle(this);
  }

  iconPath(type: string): string {
    return ICON_PATHS[type as RecursicaPaginationIconType];
  }

  iconLabel(type: string): string {
    return LABELS[type as RecursicaPaginationIconType];
  }

  setPage(page: number): void {
    if (this.disabled) {
      return;
    }
    const clamped = page <= 0 ? 1 : page > this.total ? this.total : page;
    if (this.value === undefined) {
      this._uncontrolledValue.set(clamped);
    }
    this.valueChange.emit(clamped);
  }
}
