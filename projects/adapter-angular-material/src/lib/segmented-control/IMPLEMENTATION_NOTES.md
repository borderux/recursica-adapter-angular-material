# SegmentedControl — Implementation Notes (pre-implementation stub)
...
- **Category**: EASY
- **Angular Material / CDK candidate**: `MatButtonToggleGroup`/`MatButtonToggle` (`button-toggle.d.ts`)
- **Notes**: Strong conceptual match: exclusive/multi-select button group, `appearance`, `disabled`, `value`/`(change)`. Not a literal naming match but functionally the closest thing Material has to a segmented control.
