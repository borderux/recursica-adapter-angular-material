# FileInput — Implementation Notes (pre-implementation stub)
...
- **Notes**: No file-upload/file-input component anywhere in `@angular/material` or `@angular/cdk`. Native `<input type="file">` + custom styling/behavior, same conclusion Beam reached for its own kit (Beam at least had `FileUpload`; Material has nothing).
