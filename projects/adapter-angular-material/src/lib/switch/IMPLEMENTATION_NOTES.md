# Switch — Implementation Notes (pre-implementation stub)
...
- **Angular Material / CDK candidate**: `MatSlideToggle` (`slide-toggle.d.ts`)
- **Notes**: Same gap pattern as Checkbox/Radio; `MatSlideToggle` adds `hideIcon`/`fullWidth` but still no label/description/error beyond projected content.
