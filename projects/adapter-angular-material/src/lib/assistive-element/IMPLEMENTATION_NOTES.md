# AssistiveElement — Implementation Notes (pre-implementation stub)
...
- **Angular Material / CDK candidate**: `MatHint`/`MatError` (`form-field.d.ts`)
- **Notes**: `MatHint`/`MatError` are real, but each is a directive meant to live inside `<mat-form-field>`'s content projection, not a freestanding component. Can borrow `MatFormField`'s markup/CSS conventions for text-like fields, but needs an independent implementation to serve choice controls (Checkbox/Radio/Switch) too.
