# Slider — Implementation Notes (pre-implementation stub)
...
- **Angular Material / CDK candidate**: `MatSlider`/`MatSliderThumb`/`MatSliderRangeThumb` (`slider.d.ts`, referenced from `core`)
- **Notes**: Real, MDC-redesigned slider with single and dual-thumb (range) support — but Angular's version is thumb-directive-based (`<mat-slider><input matSliderThumb></mat-slider>`) rather than a single all-in-one component, some composition needed.
