# AutoComplete — Implementation Notes (pre-implementation stub)
...
- **Angular Material / CDK candidate**: `MatAutocomplete` (+ `MatAutocompleteTrigger`) (`autocomplete.d.ts`)
- **Notes**: Real match, but Material's version is a directive (`matAutocomplete` trigger on an `<input matInput>`) plus a separate `<mat-autocomplete>` panel template — two pieces to compose rather than one drop-in component. Naming mismatch: `AutoComplete` (mantine-adapter) vs. `MatAutocomplete`.
