# NumberInput — Implementation Notes (pre-implementation stub)
...
- **Notes**: No numeric input component anywhere. `matInput` + `type="number"` on a native `<input>` gets basic browser numeric input; Mantine's clamping/formatting/increment-decrement-button parity needs full custom building.
