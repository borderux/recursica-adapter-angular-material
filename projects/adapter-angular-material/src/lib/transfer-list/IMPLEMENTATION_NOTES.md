# TransferList — Implementation Notes (pre-implementation stub)
...
- **Angular Material / CDK candidate**: _(none — compose from `MatSelectionList`/`MatListOption` + `MatButton` + `MatFormField` search)_
- **Notes**: `MatSelectionList`/`MatListOption` (`list.d.ts`) give a genuine multi-select list building block, but there's no packaged dual-list transfer component — same from-scratch composition mantine-adapter's own `TransferList` already required against Mantine. Depends on Checkbox/TextField patterns being settled first.
