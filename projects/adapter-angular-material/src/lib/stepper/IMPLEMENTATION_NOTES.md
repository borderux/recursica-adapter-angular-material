# Stepper — Implementation Notes (pre-implementation stub)
...
- **Category**: EASY
- **Angular Material / CDK candidate**: `MatStepper` (`stepper.d.ts`)
- **Notes**: Direct, rich match: `orientation` (horizontal/vertical via separate host elements `mat-horizontal-stepper`/`mat-vertical-stepper`), `labelPosition`, `headerPosition`, per-step completion/error state via `MatStep`.
