# FileUpload — Implementation Notes (pre-implementation stub)
...
- **Notes**: Same as FileInput — build fully from scratch (native file input + drag-drop via CDK's `DragDropModule` is at least available as a building block for drag-to-upload, though `@angular/cdk/drag-drop` is a general reorder/drag toolkit, not file-upload-specific).
