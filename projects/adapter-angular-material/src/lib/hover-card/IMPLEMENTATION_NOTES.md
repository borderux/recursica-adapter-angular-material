# HoverCard — Implementation Notes (pre-implementation stub)
...
- **Angular Material / CDK candidate**: _(none — build on `@angular/cdk/overlay` + hover trigger, akin to Popover)_
- **Notes**: Same toolkit as Popover; no packaged hover-triggered variant exists, needs a manual `mouseenter`/`FocusMonitor`-driven open state on top of the same Overlay primitives.
