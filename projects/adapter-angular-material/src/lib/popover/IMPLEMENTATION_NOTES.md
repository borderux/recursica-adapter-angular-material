# Popover — Implementation Notes (pre-implementation stub)
...
- **Angular Material / CDK candidate**: _(none — build on `@angular/cdk/overlay`)_
- **Notes**: No standalone `Popover` component ships, but the exact toolkit Material's own `Menu`/`Select`/`Autocomplete`/`Tooltip` are built on (`@angular/cdk/overlay`'s `Overlay`/`OverlayRef`/positioning strategies) is real, documented, and proven — real work but with a solid low-level API to build on, not starting from raw DOM.
